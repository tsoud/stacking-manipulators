...
end_eff_initial_state = pbt.getLinkState(robot_arm_ID, end_effector_id)[:2]
...
# Arrays to collect desired position and orientation end-effector
	...
	orientation_P = pbt.addUserDebugParameter('end_P', -3.14, 3.14, P_0)
	end_effector_orientation.append(orientation_P)
	orientation_Y = pbt.addUserDebugParameter('end_Y', -3.14, 3.14, Yw_0)
	end_effector_orientation.append(orientation_Y)
	orientation_R = pbt.addUserDebugParameter('end_R', -3.14, 3.14, R_0)
...
while True:
	...
	target_position = [pbt.readUserDebugParameter(p) for p in end_effector_position]
	target_orientatn = [pbt.readUserDebugParameter(o) for o in end_effector_orientation]
