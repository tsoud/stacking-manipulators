...
from robotic_stacking.rl_agents import sac_agent, td3_agent
...
class td3_trainer_AL:
    """
    Train a TD3 model with a Reverb buffer and the Actor-Learner API.

    keyword args:
    ------------
    agent_class: instance of `tfa_td3_agent` containing the agent 
        to train.
    replay_memory_class: an instance of the `reverb_buffer` or 
        `tfa_buffer` classes that hold the replay buffer, observers, 
        TF dataset and iterators.
    initial_replay_steps: number of steps for collecting initial 
        samples for the replay memory before training begins.
    random_warmup_policy: use a random policy to initially fill the up 
        the replay buffer instead of the actor policy.
    ...
    save_dir: root directory to save the model. If `None`, the model 
        is saved to a `tempdir` object.
    save_interval: how often to save the policy.
    use_tf_function: use @tf.function for the policies, which can 
        improve performance.
    """
    def __init__(self,  
                 agent:Union[sac_agent.tfa_sac_agent, td3_agent.tfa_td3_agent],
                 replay_buffer:
                    Union[reverb_buffer.reverb_buffer, replay_buffers.replay_buffer.ReplayBuffer], 
                 initial_replay_steps:int,
                 random_warmup_policy:bool=False,
        ...
        self.random_replay_warmup = random_warmup_policy
        ...
        self.use_tf_fn = use_tf_function
        # main policy:
        ...
        # policy for replay initialization:
        if self.random_replay_warmup:
