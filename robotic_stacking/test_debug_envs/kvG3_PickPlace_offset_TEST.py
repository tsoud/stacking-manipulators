...
#flip cube over
new_ort = pbt.getQuaternionFromAxisAngle([1, 0, 0], (-np.pi))
pos_A, ort_A = pbt.getBasePositionAndOrientation(cube_ID)
_, flipped_ort = pbt.multiplyTransforms(pos_A, ort_A, pos_A, new_ort)
pbt.resetBasePositionAndOrientation(
    cube_ID,
    posObj=pos_A,
    ornObj=flipped_ort
)
...
# flip target over
pos_t, ort_t = pbt.getBasePositionAndOrientation(target_location_ID)
_, flipped_ort_t = pbt.multiplyTransforms(pos_t, ort_t, pos_A, new_ort)
pbt.resetBasePositionAndOrientation(
    target_location_ID,
    posObj=pos_t,
    ornObj=flipped_ort_t
)
